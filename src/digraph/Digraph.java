...
/**
 * A directed, weighted (only positive) Graph which allows self-loops and multiedges using a HashSet
 * to store all the Vertices. To allow weighted Edges and nested Adjacency Lists we used the inner
 * classes Vertex and Edge.
 * 
 * The Verticies and Edges are reffered by key. Those Key-Values are unique. If one adds a key that
 * is already present in the Graph, the old value will be discarded.
 * 
...
public class Digraph<V, E> implements IDigraph<V, E>, Serializable {
     ...
     * 
     * @param sizeVertex sets the size of the internal vertex hashmap
     * @param sizeEdge sets the size of the internal edge hashmap
     ...
     * @param sizeVertex sets the size of the internal vertex hashmap
     * @param sizeEdge sets the size of the internal edge hashmap
     * @param loadFactorVertex sets loadFactor of the vertex hashmap
     * @param loadFactorEdge sets loadFactor of the edge hashmap
    ...
    public boolean removeEdge(E edge) {
        ...
        return (m_vList.get(e.origin).outgoingList.remove(edge) && m_vList.get(e.destination).incomingList.remove(edge) && m_eList
                .remove(edge, e));
    ...
    public E removeEdge(double weight, V origin, V destination) {
        ...
        try {
            ...
            Optional<Edge> edge = m_eList
                    .values()
                    .stream()
                    .filter(e -> (e.destination.equals(destination) && e.origin.equals(origin) && e.m_weight == weight))
                    .findFirst();
    ...
    /**
     * Representing the Vertices in the Graph. Stores all his Neighbors in a adjacencyList using a
     * HashSet.
     * 
     * @author Roman Meier, Alex Melliger, Matthias Keller, Stefan Mettler
     * 
